...
def script(name, component, default_options=None, shell='bash'):
  if shell == 'fish':
    return fish_script(name, _Commands(component), default_options)
  return bash_script(name, _Commands(component), default_options)


def bash_script(name, commands, default_options=None):
...
def fish_script(name, commands, default_options=None):
...
def visible_members(component, class_attrs=None, verbose=False):
  ...
  return [
      ...
      if member_visible(component, member_name, member, class_attrs=class_attrs,
                       verbose=verbose)
  ]
...
def Completions(component, verbose=False):
  ...
  return [
      format_for_command(member_name)
      for member_name, _ in visible_members(component, verbose=verbose)
  ]
def is_special_method(name):
    return name.startswith("__")

def is_private_method(name, verbose):
    return name.startswith("_") and not verbose

def is_excluded_member(member):
    return member in {absolute_import, division, print_function} or isinstance(member, type(absolute_import))

def member_visible(component, name, member, class_attrs=None, verbose=False):
    """Returns whether a member should be included in auto-completion or help."""
    if not isinstance(name, str) or is_special_method(name) or is_private_method(name, verbose):
        return False
    
    if verbose:
        return True
    
    if is_excluded_member(member) or (inspect.ismodule(member) and member in []):
        return False
    
    if inspect.isclass(component):
        class_attrs = class_attrs or inspectutils.GetClassAttrsDict(component) or {}
        ...
        if class_attr and isinstance(class_attr.object, tuplegetter):
            return False
    
    return True


def format_for_command(token):
...
def _Commands(component, depth=3):
  ...
  for member_name, member in visible_members(component, class_attrs={},
                                            verbose=False):
    # TODO(dbieber): Also skip components we've already seen.
    member_name = format_for_command(member_name)
...
def _GetMaps(name, commands, default_options):
  ...
  for command in commands:
    if len(command) == 1:
      ...
      arg = format_for_command(command[-1])
